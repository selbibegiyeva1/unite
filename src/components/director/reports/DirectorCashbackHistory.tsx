...
interface DirectorCashbackHistoryProps {
    period?: PeriodValue;
}

function DirectorCashbackHistory({ period: periodValue = "all" }: DirectorCashbackHistoryProps) {
    ...
    const { data, isLoading, error, refetch } = useCashbackHistory({
        page: currentPage,
        perPage: 50,
        period,
    });
    ...
    const rows: CashbackRow[] = (data?.cashback_history || [])
        .filter((item) => {
    ...
    return (
        <div className="p-5 border border-[#00000026] rounded-[16px] h-[665px] flex flex-col">
            <div className="flex items-center justify-between">
                <p className="font-medium text-[18px]">{t.directorReports.accrualTitle}</p>
                    ...
                    <table className="w-full max-1lg:min-w-[1000px]">
                        <thead>
                            <tr className="grid grid-cols-5 gap-[30px] text-center px-[13px] py-[8px] mb-4 bg-[#F5F5F9] rounded-[6px] text-[#00000099] text-[12px]">
                                ...
                                <td>{t.directorReports.refOrderId}</td>
...
export default DirectorCashbackHistory;
