...
function RegionModal({ isOpen, onClose, countryCodes }: RegionModalProps) {
    ...
    return (
        ...
        >
            <div
                className={`p-8 border-[1.5px] border-[#00000026] rounded-[24px] w-[910px] h-[700px] bg-white transform transition-transform duration-200 ${isOpen ? 'translate-y-0' : 'translate-y-2'
                    }`}
            ...
            >
                ...
                <div className="flex items-center gap-2 w-full rounded-[8px] px-5 bg-[#F5F5F9] mt-6.5 mb-[14px]">
                ...
                <div className="mt-4 flex flex-col h-[472.5px] overflow-auto transactions-table-scroll">
                    ...
                    {!isLoading && !isError && filteredCountries.map(({ code, country, extra }) => (
                        <div
                            key={code}
                            className="flex items-center rounded-[8px] justify-between gap-2 px-4 py-3 hover:bg-[#F5F5F9] transition-all duration-300 font-medium"
                        >
                            <div className="flex items-center gap-2">
                                <img
                                    ...
                                    className="w-[38px]"
                                />
                                <p>{country?.country_name.ru ?? extra?.nameRu ?? code}</p>
                            </div>
